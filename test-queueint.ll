...
entry:
  ...
  store double 5.800000e+00, double* %b
  %q1 = load %struct.QueueId** %q
  %a2 = load double* %a
  %malloccall = tail call i8* @malloc(i32 ptrtoint (double* getelementptr (double* null, i32 1) to i32))
  ...
  %b4 = load double* %b
  %malloccall5 = tail call i8* @malloc(i32 ptrtoint (double* getelementptr (double* null, i32 1) to i32))
  %tmp6 = bitcast i8* %malloccall5 to double*
  store double %b4, double* %tmp6
  %ptr7 = bitcast double* %tmp6 to i8*
  ...
  %d_ptr9 = load double* %d_ptr
  store double %d_ptr9, double* %c
  %c10 = load double* %c
  %printf = call i32 (i8*, ...)* @printf(i8* getelementptr inbounds ([4 x i8]* @fmt4, i32 0, i32 0), double %c10)
  ...
  %d_ptr14 = bitcast i8* %val_ptr13 to double*
  %d_ptr15 = load double* %d_ptr14
  store double %d_ptr15, double* %d
  %d16 = load double* %d
  %printf17 = call i32 (i8*, ...)* @printf(i8* getelementptr inbounds ([4 x i8]* @fmt4, i32 0, i32 0), double %d16)
  ret i32 0
}
